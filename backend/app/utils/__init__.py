...
def _build_cdss_payload(
    ...
    if triage_level == "Low Priority":
        ...
        if base_level == "Low Priority":
            ...
            triage_reasons.append(
                "Clinical baseline protocol requires medical evaluation"
            )
        else:
        ...
        if has_warning_signs:
            triage_reasons.append(
                "Severe warning signs detected (requires immediate clinical assessment)"
            )
    ...
    if not is_uncertain:
        if disease == "Dengue":
            if (
                priority_rank[triage_level] >= 2
                and base_level == "Low Priority"
                and not has_warning_signs
            ):
                triage_reasons.append(
                    "Suspected Dengue requires an initial laboratory test (CBC) and medical monitoring."
                )
            actions.insert(0, "Get a Complete Blood Count (CBC) as soon as possible.")
            actions.insert(
                0,
                "Do not take ibuprofen or aspirin (NSAIDs) as they increase bleeding risk. Take paracetamol for fever.",
            )
            ...
            if (
                ...
                triage_reasons.append(
                    "Requires clinical evaluation for appropriate prescription antibiotics."
                )
            ...
            if (
                ...
                triage_reasons.append(
                    "Requires clinical evaluation for Vitamin A supplementation and isolation protocols."
                )
            actions.insert(0, "Isolate immediately to prevent spreading the virus.")
        elif disease == "Diarrhea":
            actions.insert(
                0,
                "Begin drinking Oral Rehydration Salts (ORS) immediately to replace lost fluids.",
            )
