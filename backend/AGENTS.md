...
To start the development server, run the `run.py` script. The `run_flask.bat` file provides a convenient way to do this on Windows.
...
- **API Structure:** The application follows a standard Flask app-factory structure. Routing and bootstrap wiring are initialized from `run.py` via `app/__init__.py`.
- **Machine Learning:** The machine learning models are loaded and managed within `app/services/ml_service.py`. The `MCDClassifierWithSHAP` class encapsulates the prediction and uncertainty logic.
