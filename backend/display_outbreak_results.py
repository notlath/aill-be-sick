...
def display_results(url="http://localhost:10000/api/surveillance/outbreaks?summary=true"):
    ...
    print(f"  Contamination Threshold:     {data.get('contamination', 0.05) * 100:.1f}%")
    print(f"  Outbreak Alert:              {'⚠️  YES - ALERT!' if data.get('outbreak_alert') else '✅ NO - Normal'}")
    print()
    ...
    if top_anomalies:
        for i, anomaly in enumerate(top_anomalies, 1):
            ...
            print(f"    Location:         {anomaly.get('city', 'Unknown')}, {anomaly.get('region', 'Unknown')}")
            print(f"    Coordinates:      ({anomaly.get('latitude', 0):.4f}°, {anomaly.get('longitude', 0):.4f}°)")
    ...
    print("  • Anomaly Score: Negative = unusual pattern. Closer to -1 = more anomalous")
