...
- **User Flow**:
  ...
  2. **Sign Up action**: If they click "Register patient account", the `patientSignup` server action is invoked. A verification email is sent, an initial profile is saved to the database, and the user is redirected to the `/verify-email` holding page.
