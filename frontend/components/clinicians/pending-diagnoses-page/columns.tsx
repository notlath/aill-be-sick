...
export type PendingDiagnosisRow = {
  ...
  createdAt: Date;
  patientId: string;
  reliability: ReturnType<typeof getReliability>;
  submittedAt: Date;
  user?: User;
  notes?: DiagnosisNoteRow[];
};

export const pendingDiagnosesColumns: ColumnDef<PendingDiagnosisRow>[] = [
  {
    accessorKey: "disease",
    header: ({ column }) => (
      <button
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <span className={`badge ${reliability.badgeClass} badge-sm whitespace-nowrap`}>
          {reliability.label}
        </span>
      );
    },
