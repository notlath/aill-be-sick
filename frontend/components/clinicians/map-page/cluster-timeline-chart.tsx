...
type ClusterTimelineChartProps = {
  patients: Patient[];
  nClusters: number;
...
type TimeBucket = {
  ...
  periodStart: string;
  periodEnd: string;
  [key: string]: string | number; // cluster_0, cluster_1, etc.
};
...
export function ClusterTimelineChart({
  patients,
  nClusters,
  selectedCluster,
  clusterColorIndex,
}: ClusterTimelineChartProps) {
  ...
  const chartData = useMemo<TimeBucket[]>(() => {
    // Filter to selected cluster patients with diagnosed_at
    const withDates = patients.filter(
      (p) => p.diagnosed_at && p.cluster === selectedCluster,
    );
    ...
    for (const patient of withDates) {
      const key =
        granularity === "week"
          ? getISOWeekKey(patient.diagnosed_at!)
          : getMonthKey(patient.diagnosed_at!);
    ...
    <div className="bg-base-200 rounded-box p-4">
      <div className="flex items-center justify-between mb-3">
        <p className="font-semibold text-sm">
          Cluster {selectedCluster} — Patients Over Time
        </p>
        <Select
          value={granularity}
