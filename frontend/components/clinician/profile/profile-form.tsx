...
import { updateEmailAction, updatePasswordAction } from "@/actions/update-credentials";
...
export default function ClinicianProfileForm({ user: initialUser }: ProfileFormProps) {
  ...
  const { execute: executeUpdateEmail, isExecuting: isUpdatingEmail } = useAction(
    updateEmailAction,
    {
      onSuccess: ({ data }) => {
        if (data?.success) toast.success("Email updated successfully - check your inbox to confirm.");
        else if (data?.error) toast.error(data.error);
      },
  ...
  const { execute: executeUpdatePassword, isExecuting: isUpdatingPassword } = useAction(
  ...
  // Handlers
  const handleProfileSubmit = useCallback((e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    ...
    <div className="space-y-10">
        ...
        <div className="relative px-8 pb-8">
          ...
          <div className="relative -mt-16 mb-6">
            <div className="relative inline-block">
              ...
              <label className={`absolute bottom-0 right-0 w-10 h-10 rounded-full bg-primary hover:bg-primary/90 flex items-center justify-center cursor-pointer shadow-md transition-all duration-200 ${isUploadingAvatar ? 'opacity-50 cursor-not-allowed' : ''}`}>
                {isUploadingAvatar ? (
          ...
          <form onSubmit={handleProfileSubmit} className="grid gap-6 md:grid-cols-2">
            <div className="space-y-2">
              <label className="text-sm block font-medium text-base-content">Name</label>
              <Input
                name="name"
                value={name}
            ...
            <div className="space-y-2 w-full">
              <label className="text-sm block font-medium text-base-content">Gender</label>
              <Select className="w-full" value={gender} onValueChange={(value) => setGender(value as "MALE" | "FEMALE" | "OTHER")}>
            ...
            <div className="space-y-2">
              <label className="text-sm block font-medium text-base-content">Birthday</label>
              <Input
                type="date"
                value={birthday}
            ...
            <div className="md:col-span-2 flex justify-end">
              <button type="submit" disabled={isUpdatingProfile} className="btn btn-primary rounded-[10px] px-6 min-w-[100px]">
      ...
      <section className="bg-white/80 backdrop-blur-sm rounded-2xl border border-border shadow-sm p-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-xl font-semibold text-base-content">Account Credentials</h2>
            <p className="text-sm text-muted">Update your email address or password.</p>
          </div>
        </div>
        ...
        <div className="grid gap-8 md:grid-cols-2">
          ...
          <form onSubmit={handleEmailSubmit} className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm block font-medium text-base-content">Email Address</label>
          ...
          <form onSubmit={handlePasswordSubmit} className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm block font-medium text-base-content">New Password</label>
              <div className="relative">
                <Input
                  type="password"
                  value={password}
